...
| `ECOURT_DL_TIMEOUT` | (선택) 사건 1건 다운로드 완료 대기 최대 초. 대용량이 잘리면 늘림 (기본 `900`) |
...
> ⚠️ **데몬은 컴퓨터가 켜져 있어야 동작합니다.** 매일 `SYNC_TIME` 시각에 PC가
> **켜져 있고 로그인된 상태**여야 실행됩니다. 전원이 꺼져 있거나 절전/최대절전
> 상태면 그날은 건너뜁니다. 항상 받으려면 PC를 켜 두거나(노트북은 전원 연결 + 절전
> 해제), 해당 시각에 컴퓨터를 켜 두세요. (잠긴 화면·원격 세션에서도 GUI 자동화가
> 안 되니, 로그인된 데스크톱 세션이 필요합니다.)
...
## 잘 안 될 때 / Troubleshooting — Claude에게 물어보기

오류가 나거나 동작이 이상하면, **Claude Code**(또는 claude.ai)에 아래 내용을 붙여넣고
물어보세요. 저장소 링크를 함께 주면 Claude가 코드·구조를 파악해 정확히 도와줍니다.

```text
ecourt-cli 로 전자소송 사건기록 자동 다운로드를 쓰는 중인데 잘 안 돼.
- 저장소(코드/구조 참고): https://github.com/jurisupport/ecourt-cli
- 실행한 명령: (예: .\.venv\Scripts\python.exe ecourt_download.py --case 0)
- 화면에 나온 오류 / logs 폴더의 최신 .log 내용: (붙여넣기)
- output 폴더의 최근 스크린샷도 함께 봐줘.
원인과 해결 방법을 한국어로 단계별로 알려줘.
```

> 팁: `logs\` 의 가장 최근 `*.log` 와 `output\` 의 최근 스크린샷(`*.png`)이 원인 파악에
> 가장 도움이 됩니다. (비밀번호·토큰은 암호화되어 있어 로그에 평문으로 남지 않습니다.)

---

## 관련 도구 / Related tools (JuriSupport)

전자소송 다운로드 다음 단계의 업무도 자동화하고 싶다면:

| 도구 | 설명 | 설치 (PowerShell) |
|------|------|-------------------|
| **[jurisupport-plugins](https://github.com/jurisupport/jurisupport-plugins)** | Claude Code 법률 자동화 패키지 — 사건폴더로 사실관계 요약·쟁점 도출·법령/판례 검증·서면 초안 생성 | `irm https://raw.githubusercontent.com/jurisupport/jurisupport-plugins/main/windows-bootstrap.ps1 \| iex` |
| **[legal-terminal](https://github.com/jurisupport/legal-terminal)** | 소송 변호사용 올인원 AI 작업공간 (Claude 터미널 + 문서 에디터 + 기록 뷰어 + 사건 대시보드) | `irm https://raw.githubusercontent.com/jurisupport/legal-terminal/main/install.ps1 \| iex` |
| **[jurisupport.com](https://jurisupport.com)** | 법률 실무 관리 플랫폼 (AI 문서 에디터 · 콘텐츠 스튜디오 · 사건 관리 · MCP 연결) | 웹 |
